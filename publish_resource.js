...
Page({
  data: {
    // 这些分的专业不一定都在讨论区中，大致上是这样
    leftMenu: ['未选择', '计算机类', '电子类', '数学类', '电信类', '建筑类', '光电类', '机械类', '材料类', '其他'],
      ...
      const res = await db.collection('resource')
        .add({
          data: {
            ...
            class: this.data.leftIndex === this.data.leftMenu.length - 1 ? -1 : this.data.leftIndex - 1 
          }
        });
